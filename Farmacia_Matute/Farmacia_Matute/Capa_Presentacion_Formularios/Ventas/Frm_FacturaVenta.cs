...
{
    public partial class Frm_FacturaVenta : Form
    {
        ...
        Ng_FacturasVentas facturas = new Ng_FacturasVentas();
        ...
        private void AnalizaDatosPaciente(DataTable tabla)
        {
            if (tabla.Rows.Count == 1)
            {
                AsignarDatosPaciente(tabla);
        ...
        private void AsignarDatosPaciente(DataTable tabla)
        {
            txt_apellido_pac.Text = tabla.Rows[0]["Apellido"].ToString();
            txt_nombre_pac.Text = tabla.Rows[0]["Nombre"].ToString();
        }
        ...
        private void Frm_FacturaCompra_Load(object sender, EventArgs e)
        {
            ...
            cmb_tipodoc_pac.Cargar(tipo.EstrCombo());
        ...
        private void btn_grabar_Click(object sender, EventArgs e)
        {
            ...
            if (facturas.VerificarFacturacion(txt_nrofac.Text,txt_cuitfarmacia.Text,cmb_tipo_factura.SelectedValue.ToString()) == Ng_FacturasVentas.Facturacion.no_existe)
            {
                facturas.insertar(txt_cuitfarmacia.Text, txt_cuitlaboratorio.Text, cmb_tipo_factura.SelectedValue.ToString(),
                    txt_nrofac.Text, cmb_tipo_doc.SelectedValue.ToString(), txt_nrodoc.Text,cmb_tipodoc_pac.SelectedValue.ToString(),txt_doc_pac.Text, grid_detalle, lblt_fecha.Pp_Text.ToString());
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (txt_doc_pac.Text != "")
            {
                AnalizaDatosPaciente(emp.buscar_Empleado_Paciente(cmb_tipodoc_pac.SelectedValue.ToString(),txt_doc_pac.Text,true));
            }
            else
                MessageBox.Show("No cargo el numero de documento del Paciente");
